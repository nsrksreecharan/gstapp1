...
function RouterProvider({
  ...
  let [state, setStateImpl] = React3$1.useState(router.state);
  ...
  let logErrorsAndSetState = React3$1.useCallback(
    (newState) => {
    ...
    (newState, { deletedFetchers, flushSync, viewTransitionOpts }) => {
      ...
      if (!viewTransitionOpts || !isViewTransitionAvailable) {
        if (reactDomFlushSyncImpl && flushSync) {
          ...
          React3$1.startTransition(() => logErrorsAndSetState(newState));
        }
        return;
      }
      if (reactDomFlushSyncImpl && flushSync) {
